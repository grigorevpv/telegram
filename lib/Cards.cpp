...
char *reader() {
    ...
    debug = 1;
    char *id = (char*)malloc(23);
    memset(id, '0', 23);
    char *error = (char*)malloc(4);
    memset(error, '0', 4);
    ...
    if (find_config_param("GPIO=",str,sizeof(str)-1,1)==1) {  //  считываем из файла конфига необходимые параметры
        gpio=(uint8_t)strtol(str,NULL,10);      // преобразование строки в числовое значение по основанию 10
        if (gpio<28) {
            use_gpio=1;
        } else {
            gpio=255;
    ...
    if (HW_init(spi_speed,gpio)) return error; // Если не удалось инициализировать RC522 выходим.
    if (read_conf_uid(&uid)!=0) return error;
    setuid(uid); // установка идентификатора владельца текущего процесса
    InitRc522(); // сброс параметров платы и включение антенны считывателя
        ...
        *(p++)=0;
        ...
        if (find_config_param(sn_str,str,sizeof(str),1)>0) {
            ...
            if (debug) {
                ...
                strcpy(id, sn_str);																		// добавил объявление строки
            }
